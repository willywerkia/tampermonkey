...
(() => {
  ...
  function createRuntime({ label: label2 }) {
    ...
    let disposed = false;
    const assertActive = () => {
      if (disposed) throw new Error(`${label2} Toolbox Runtime wurde bereits beendet.`);
    };
    return {
  ...
  function bootstrapToolbox({ label: label2, marker }, features) {
    if (window.top !== window) return null;
    const root = document.documentElement;
    if (!root || root.getAttribute(marker) === "true") return null;
    root.setAttribute(marker, "true");
    const runtime = createRuntime({ label: label2 });
    features.forEach(({ id, execute }) => {
      try {
        execute(runtime);
      } catch (error) {
        console.error(`[${label2} Toolbox] Feature konnte nicht starten: ${id}`, error);
      }
    });
