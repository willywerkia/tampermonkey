...
(() => {
  ...
  var MARK_OPTIONS = { prefixByColor: { [RED]: "❌ ", [YELLOW]: "⚠️ " } };
  function comparisonValue2(value, original) {
    return comparisonValue(value, original, MARK_OPTIONS);
  }
  function langRank(v) {
  ...
  function executeQuestionnaireEvaluation(runtime) {
    ...
    function createOrUpdateOmNotes(dialog) {
      ...
      const omField = getFieldsByLabel(dialog, "Zusätzliche Information - OM")[0];
      if (!omField) return;
  ...
  function executeRouteCalculation(runtime) {
    ...
    const isQuestionnaireDialog2 = (dialog) => {
      const text = dialog?.innerText || dialog?.textContent || "";
